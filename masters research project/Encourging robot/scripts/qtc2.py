...
class OnlineQtcCreator(object):
    ...
    def __init__(self, name):
        ...
        self.pub = rospy.Publisher("~QTC", String, queue_size=1)
        ...
        rospy.Subscriber("/json_example/json_out", String, self.boo)
    ...
    def boo(self, msg):
        print "I received:", msg.data, type(msg.data) # Incoming string
        my_dict = json.loads(msg.data) # Make it a dictioanry again
        for k, v in my_dict.items():
            print "%s:" % str(k), v, type(v)
            
    def send_dict(self, data):
        data = json.dumps(data) # Serialise data ro create string
        #print "sending data"
        self.pub.publish(data) # Publish string
